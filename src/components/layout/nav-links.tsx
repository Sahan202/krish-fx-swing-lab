...
export default function NavLinks({ role }: { canManage: boolean; role?: string }) {
  const pathname = usePathname() ?? ''; const [open, setOpen] = useState(false); const allLinks = role === 'student' ? links.filter(([href]) => href === '/dashboard' || href === '/live') : links; const isActive = (href: string) => href === '/' ? pathname === '/' : pathname.startsWith(href);
