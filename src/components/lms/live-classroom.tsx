...
export default function LiveClassroom() {
  ...
  return <main className="min-h-screen bg-[#07111f] px-5 py-8 text-white"><div className="mx-auto max-w-5xl">
    <div className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.2em] text-cyan-300">Protected Zoom access</p><h1 className="mt-2 text-3xl font-bold">Live classes</h1><p className="mt-2 text-sm text-slate-400">Only approved students enrolled in the course can open a class.</p></div><Link href="/dashboard" className="text-sm font-semibold text-amber-300">Back to dashboard</Link></div>
    ...
    <div className="mt-8 grid gap-4 md:grid-cols-2">{sessions.map((session) => <article key={session.id} className="rounded-2xl border border-white/10 bg-white/[.04] p-6">
      ...
      <a href={`/api/live/${session.id}/join`} className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-cyan-400 px-4 py-3 font-bold text-slate-950 transition hover:bg-cyan-300">Join on Zoom <ExternalLink className="size-4" /></a>
    </article>)}{!sessions.length && !message && <p className="rounded-2xl border border-dashed border-white/10 p-8 text-center text-slate-400 md:col-span-2">No Zoom classes are scheduled for your courses.</p>}</div>
  </div></main>;
}
