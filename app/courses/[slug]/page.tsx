...
export default async function CoursePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data: permissions } = lessons.length ? await supabase.from('student_video_permissions').select('lesson_id').eq('student_id', user.id).in('lesson_id', lessons.map((lesson) => lesson.id)) : { data: [] };
  const permittedIds = new Set((permissions ?? []).map((permission) => permission.lesson_id));
  const restrictedLessonIds = permittedIds.size ? lessons.filter((lesson) => !permittedIds.has(lesson.id)).map((lesson) => lesson.id) : [];
  return <><div className="lms-course-overview bg-[#07111f] px-5 py-8 text-white sm:px-6"><div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-5"><div><p className="text-sm text-amber-400">{course.level}</p><h1 className="mt-2 text-3xl font-bold">{course.title}</h1></div><EnrollButton courseId={course.id} enrolled={Boolean(enrollment)} /></div><p className="mx-auto mt-4 max-w-7xl text-slate-400">{course.description}</p></div><VdoCipherLessonPlayer course={{ title: course.title, description: course.description, level: course.level }} lessons={lessons} sections={sections ?? []} completedLessonIds={(progress ?? []).filter((item) => item.completed).map((item) => item.lesson_id)} restrictedLessonIds={restrictedLessonIds} /></>;
}
