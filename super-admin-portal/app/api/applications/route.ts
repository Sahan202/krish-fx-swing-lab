...
export async function PATCH(request: NextRequest) {
  ...
  if (body.action === 'reject') { const { error: updateError } = await admin.from('student_applications').update({ status: 'rejected' }).eq('id', body.id); return updateError ? NextResponse.json({ error: updateError.message }, { status: 400 }) : NextResponse.json({ status: 'rejected' }); }
