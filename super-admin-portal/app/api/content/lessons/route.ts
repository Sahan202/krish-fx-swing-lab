...
export async function PATCH(request: NextRequest) {
  try { const actor = await requirePermission(request, 'manage_content'); if (!actor) return NextResponse.json({ error: 'You do not have lesson/content permission.' }, { status: 403 }); const body = await request.json(); const values = { title: body.title, description: body.description || null, vdocipher_video_id: body.videoId || null, lesson_order: Number(body.order) || 1 }; const { error } = await serviceClient().from('lessons').update(values).eq('id', body.id); if (error) return NextResponse.json({ error: error.message }, { status: 400 }); await audit(actor, 'LESSON_UPDATED', 'lesson', body.id, values); return NextResponse.json({ ok: true }); }
