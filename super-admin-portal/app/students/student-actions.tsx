...
export default function StudentActions({ id }: { id: string }) {
  ...
  async function reset() { if (!window.confirm('Generate a new password for this student?')) return; setBusy(true); setMessage(''); const response = await request('/api/students/reset','POST'); const data = await response.json(); setMessage(response.ok ? `New password: ${data.password}` : data.error); setBusy(false); }
